...
// accepted_answers takes priority over answer (single-answer fallback)
...
function renderQuestionCard(question, { answered = false, userAnswer = null, progress = null } = {}) {
  ...
  const isCorrect = answered && isAns(userAnswer, question);
  const isWrong = answered && userAnswer !== null && !isAns(userAnswer, question);
  ...
  const choicesHtml = `
    <ul class="choices-list" aria-label="선택지">
      ${renderChoices(choices || [], answered, answer, userAnswer, question)}
    </ul>`;
  ...
  if (answered) {
    if (isCorrect) {
      ...
      resultHtml = `<div class="result-indicator wrong show" role="alert">
        <span aria-hidden="true">✗</span> 오답입니다. 정답: ${answersOf(question).join(', ')}번
      </div>`;
    }
  }
