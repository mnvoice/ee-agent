...
(function() {
  ...
  // Multi-answer helpers: accepted_answers takes priority over answer
  function answersOf(q) {
    return (q.accepted_answers && q.accepted_answers.length) ? q.accepted_answers : [q.answer];
  }
  function isAns(num, q) { return answersOf(q).indexOf(num) >= 0; }

  function renderChoices(choices, answered, correctAnswer, userAnswer, qObj) {
    return choices.map(function(text, idx) {
      var num = idx + 1;
      var cls = 'choice-btn';
      if (answered) {
        if (qObj ? isAns(num, qObj) : num === correctAnswer) cls += ' correct';
        else if (num === userAnswer && (qObj ? !isAns(num, qObj) : num !== correctAnswer)) cls += ' wrong';
      }
  ...
  function renderQuestionCard(question, opts) {
    ...
    var isCorrect = answered && isAns(userAnswer, question);
    var isWrong = answered && userAnswer !== null && !isAns(userAnswer, question);
    ...
    var choicesHtml = '<ul class="choices-list" aria-label="\uC120\uD0DD\uC9C0">' +
      renderChoices(choices || [], answered, answer, userAnswer, question) + '</ul>';
    ...
    if (answered) {
      if (isCorrect) {
        ...
        resultHtml = '<div class="result-indicator wrong show" role="alert"><span aria-hidden="true">\u2717</span> \uC624\uB2F5\uC785\uB2C8\uB2E4. \uC815\uB2F5: ' + answersOf(question).join(', ') + '\uBC88</div>';
      }
    }
