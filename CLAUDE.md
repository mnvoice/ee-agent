...
### Purpose
Electrical Engineer (전기기사) exam auto-solver. 4-Agent pipeline system.

### Current Performance
- 2020 Exam 1: **75.0%** (60/80) — passing
- 2022 Exam 2: **92.5%** (74/80) — passing
- PWA App: **5,007 complete questions** (filtered from 5,762)

### LLM Configuration
- Logic Solver: **Claude API** (solver_llm = anthropic)
- Local fallback: qwen2.5:7b (Ollama, ~42% accuracy ceiling)

### Execution
```bash
python3 scripts/run_sample.py --pdf data/20200424_1회.pdf --year 2020
python3 scripts/seed_knowledge.py  # Rebuild RAG DB
python3 scripts/build_app_data.py  # Rebuild app data (filters incomplete)
python3 scripts/recover_from_mathpix.py  # Recover from Mathpix OCR
```

### Core Design
- PDF 2-column split: `_extract_two_column_text()` (page.width/2)
- Question type detection: calc (numbers+units) vs concept (text)
- RAG: TF-IDF (data/knowledge_store.json, 884 sections)
- Prompt: `"selected_choice": answer_number` (no numeric examples in placeholders)

### PWA App Features (app/)
- 5,007 questions with 3-step solutions + SVG diagrams
- CBT mock exam (100Q random, 150min timer, pass/fail scoring)
- Concept-based group study (tag progress tracking)
- Weakness analysis (top 10 weak concepts by error rate)
- Apple Pencil annotation, KaTeX math rendering, offline PWA

### Cautions
- Subject.ELECTROMAGNETISM = "전기자기학" (was "전기이론" — changed)
- MAX_CONCURRENT=1 (Ollama cannot handle concurrent requests)
- results.json is overwritten on each pipeline run
- Data files (data/) are in .gitignore — not tracked by git
